...
pub struct WorldStateTracker {
    ...
    pub conflicts: Vec<(WorldState<WorldFact, WorldValue>, WorldState<WorldFact, WorldValue>)>,
}
...
impl WorldStateTracker {
    ...
    pub fn update(&mut self, game: &Game) -> GameResult<()> {
        ...
        if !self.states.is_empty() && state.has_conflicting_facts(self.states.last().unwrap().clone())
        {
            self.conflicts.push((state.clone(), self.states.last().unwrap().clone()));
        }
        self.states.push(state);
        Ok(())
