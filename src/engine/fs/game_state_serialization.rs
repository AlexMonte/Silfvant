...
//Event for loading the game
#[derive(Debug, Clone, Eq, PartialEq, Resource, Serialize, Deserialize, Default)]
pub(crate) struct GameLoadRequest {
    pub(crate) filename: Option<String>,
}

//Model for saving the game
...
struct SaveModel {
    ...
    // ai_agents_tranform: Vec<Transform>,
    // items_transform: Vec<Transform>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    dialog_event: Option<DialogEvent>,
}

impl Serialize for SaveModel {
    fn serialize(&self, path: &Path) -> Result<String> {
        ...
        fs::write(&path, self).unwrap_or_else(|e| anyhow!("Failed to write save {filename}: {e}"));
    }

    fn deserialize(&self, path: &Path) -> Result<Self> {
